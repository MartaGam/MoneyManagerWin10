...
namespace MoneyManager.Controllers
{
	public class TransactionsController : Controller
	{
		...
		[HttpPost]
		public async Task<ActionResult> ChangeBalance(int newValue)
		{
			...
			SqlCommand myCommand = new SqlCommand($"UPDATE Users SET Ballance = {newValue} WHERE Id = {user.Id}", myConnection);
		...
		public async Task<ActionResult> Add(AddTransactionModel transaction)
		{
			...
			SqlCommand myCommand = new SqlCommand(
				"INSERT INTO Transactions (Id, Coast, CollaboratorsIds, CreationDate, DeadlineDate, Description, FinishedIds, InProgressIds, OwnerId, Title, OngoingDate) " +
				$"Values ({data.Id},'{data.Coast}', '{data.CollaboratorsIds}', '{data.CreationDate}', '{data.DeadlineDate}', N'{data.Description}'," +
				$"'{data.FinishedIds}', '{data.InProgressIds}', '{data.OwnerId}', N'{data.Title}', '{data.OngoingDate}')", myConnection);
		...
		public async Task<ActionResult> Collaborate(int id)
		{
			...
			await myConnection.OpenAsync();
			SqlCommand updateTokenCommand = new SqlCommand(
				$"UPDATE Transactions SET InProgressIds = '{JsonConvert.SerializeObject(inProgressIds)}' WHERE Id = '{transaction.Id}'", myConnection);
		...
		public async Task<ActionResult> Approve(int transactionId, int userId)
		{
			...
			SqlCommand updateTokenCommand = new SqlCommand(
				$"UPDATE Transactions SET FinishedIds = '{JsonConvert.SerializeObject(finishedIds)}' WHERE Id = '{transaction.Id}'", myConnection);
			await updateTokenCommand.ExecuteNonQueryAsync();

			SqlCommand updateInProgressCommand = new SqlCommand(
				$"UPDATE Transactions SET InProgressIds = '{JsonConvert.SerializeObject(inProgressIds)}' WHERE Id = '{transaction.Id}'", myConnection);
		...
		public async Task<ActionResult> ApproveAll(int friendId)
		{
			...
			{
				var collaboratorsIds = JsonConvert.DeserializeObject<int[]>(transaction.CollaboratorsIds).ToList();
				var finishedIds = JsonConvert.DeserializeObject<int[]>(transaction.FinishedIds).ToList();
				var inProgressIds = JsonConvert.DeserializeObject<int[]>(transaction.InProgressIds).ToList();
				...
				SqlCommand updateFinishedCommand = new SqlCommand(
					$"UPDATE Transactions SET FinishedIds = '{JsonConvert.SerializeObject(finishedIds)}' WHERE Id = '{transaction.Id}'", myConnection);
